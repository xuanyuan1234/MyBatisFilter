package com.wanke.convert.config;
...
/**
 * 拦截 PL/SQL语句
 * 只拦截 update类型，并带有 CALLABLE 标记的的SQL语句
 */
@Component
...
public class UserAuthorityDataInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        // 获取到最终的sql语句
        String mSql = getSql(configuration, boundSql, sqlId);

        StatementType statementType = mappedStatement.getStatementType();

        mSql = mSql.replace(String.format("%s:", sqlId), "").replace("begin", "").replace("end;", "");

        mSql = String.format("call proc_mysql(\"%s\")", mSql);
...
//        field.set(boundSql, mSql);
        ...
        System.out.println(String.format("statementType:%s", statementType));
        if ("CALLABLE".equals(statementType.toString())) {
            return jdbcTemplate.update(mSql);
        } else {
            return invocation.proceed(); //正常执行
        }
    }
